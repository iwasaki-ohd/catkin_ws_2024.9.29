...
def callback(data, callback_args):
    global JG, pc_data1, pc_data2, pc_data3 # xxx
    ...
    if pc_name == "pc1":
        #rospy.loginfo(f"テスト1:clear, {pc_data}")
        pc_data1 = pc_data
        rospy.loginfo(f"テスト1:移行, {pc_data1}") # このデバッグログを消すとうまく値の受け渡しができなくなる()
    if pc_name == "pc2":
        #rospy.loginfo(f"テスト2:clear, {pc_data}")
        pc_data2 = pc_data
        rospy.loginfo(f"テスト2:移行, {pc_data2}") 
    if pc_name == "pc3": # xxx
        #rospy.loginfo(f"テスト3:clear, {pc_data}")
        pc_data3 = pc_data
        #if pc_data3 is not None:
         #   pass
        #rospy.loginfo(f"テスト3:移行, {pc_data3}")  # xxx
    ...
    if JG == 0: # パブリッシュをまだ行っていない場合(1回の処理で複数回パブリッシュされないようにするための条件文)
        ...
        if (len(data_dict['pc1']) > 0 and len(data_dict['pc2']) > 0 and len(data_dict['pc3']) > 0): # xxx
            ...
            # 統合リストと照合して、それぞれのデータを一つのStringメッセージでパブリッシュ。
            # ↑面倒だったので別の方法でやる(5.1)
            ...
            #JG += 0
            rospy.loginfo(f"受信したpc2のデータ：{pc_data2}")
            ...
            #rospy.loginfo(f"デバッグ：{len(pc_data2)/5}")
            for i in range(int(len(pc_data2)/5)):# 検出した人数分ループ
                for id_mapping in id_mappings_pc2:
                    original_id, merged_id = id_mapping
                    #rospy.loginfo(f"デバッグ　　original_id:{original_id}、merged_id:{merged_id}")
                    if pc_data2[i * 5] == original_id:
                        rospy.loginfo(f"pc2：ID {original_id} は既に統合されています。統合後のID {merged_id} に変更します。")
                        pc_data2[i * 5] = merged_id
                        judgement_2 += 1
            if judgement_2 > 0:
                rospy.loginfo(f"統合完了！ 統合後のデータ：{pc_data2}")
            else:
                rospy.loginfo(f"統合はなし")

            str_pc_data = ""
            list_a = [val for index, val in enumerate(pc_data2) if index % 5 != 4] # 距離データを取り除く
            for item in list_a:
                str_pc_data += str(item) + ","
            str_pc_data = str_pc_data[0:-1] # 先頭の"["と末尾の"],"を取り除く
            pub_integration_data.publish(str_pc_data)
            ...
            #JG += 0
            rospy.loginfo(f"受信したpc3のデータ：{pc_data3}")
            rospy.loginfo(f"---統合済みIDの捜索---　　id_mappings_pc3：{id_mappings_pc3}")
            judgement_3 = 0 # 統合したかどうかの判定用変数
            #rospy.loginfo(f"デバッグ：{len(pc_data3)/5}")
            for i in range(int(len(pc_data3)/5)):# 検出した人数分ループ
                for id_mapping in id_mappings_pc3:
                    original_id, merged_id = id_mapping
                    #rospy.loginfo(f"デバッグ　　original_id:{original_id}、merged_id:{merged_id}")
                    if pc_data3[i * 5] == original_id:
                        rospy.loginfo(f"pc3：ID {original_id} は既に統合されています。統合後のID {merged_id} に変更します。")
                        pc_data3[i * 5] = merged_id
                        judgement_3 += 1
            if judgement_3 > 0:
                rospy.loginfo(f"統合完了！ 統合後のデータ：{pc_data3}")
            else:
                rospy.loginfo(f"統合はなし")

            str_pc_data = ""
            list_a = [val for index, val in enumerate(pc_data3) if index % 5 != 4] # 距離データを取り除く
            for item in list_a:
                str_pc_data += str(item) + ","
            str_pc_data = str_pc_data[0:-1] # 先頭の"["と末尾の"],"を取り除く
            pub_integration_data.publish(str_pc_data)
