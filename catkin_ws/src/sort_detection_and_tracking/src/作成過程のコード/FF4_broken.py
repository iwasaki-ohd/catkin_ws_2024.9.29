...
# integration_data = None # パブリッシュするデータをまとめる変数
    ...
    if(S == 0): # 許容時間差に収まるデータの組み合わせが見つからなかった場合
        ...
        if matched_data_pc1 is not None and matched_data_pc2 is not None:
            ...
            for j in range(0, int(len(matched_data_pc2)/3)):# 検出した人数分ループ
                rospy.loginfo(f"id_mappings_pc2：{id_mappings_pc2}")
            ...
            for i in range(0, int(len(matched_data_pc1)/3)):# 検出した人数分ループ
                for j in range(0, int(len(matched_data_pc2)/3)):# 検出した人数分ループ
                    if matched_data_pc1[i * 3] == matched_data_pc2[j * 3]:
                        ...
                        rospy.loginfo(f"同一IDを発見。{matched_data_pc1[i * 3]}, {matched_data_pc2[j * 3]}")
            # IDを昇順に並べる
            ...
            for i in range(0, int(len(sorted_pc1_del_index_list))):
            ...
            for j in range(0, int(len(sorted_pc2_del_index_list))):
            ...
            remained_data_pc1 = matched_data_pc1
            ...
            for i in range(0, int(len(matched_data_pc1)/3)):# 検出した人数分ループ
                ...
                for j in range(0, int(len(matched_data_pc2)/3)):# 検出した人数分ループ
                    ...
                    current_distance = distance(matched_data_pc1[i * 3 + 1: i * 3  + 1 + 2],
                                                matched_data_pc2[j * 3 + 1: j * 3  + 1 + 2])
                ...
                if change_num_pc1 is not None and change_num_pc2 is not None and matched_data_pc1[change_num_pc1] != matched_data_pc2[change_num_pc2]: # マッチしたデータのIDが同一でない場合
                    ...
                    # 残りデータの更新(統合データの削除)
                    del remained_data_pc1[change_num_pc1:change_num_pc1+3]

            # IDを統合しなかった(それぞれのカメラでしか認識していない)人間のデータを追加
            
            for j in range(0, int(len(matched_data_pc2)/3)):# 残った人数分ループ
                integration_data += convert_to_string(matched_data_pc2[i:i+3])
                rospy.loginfo(f"integration_data: {integration_data}")
