...
#******************************************************************↑ここがうまく機能していない
            ...
            rospy.loginfo(f"デバッグ：{len(pc_data)/3}")
            for i in range(int(len(pc_data)/3)):# 検出した人数分ループ
...
def process_data(data_dict):
    ...
    #detecting_num = len(matched_data_pc1)/3 + len(matched_data_pc2)/3
    ...
    if(S == 0): # 許容時間差に収まるデータの組み合わせが見つからなかった場合
        ...
        if matched_data_pc1 is not None and matched_data_pc2 is not None:
            ...
            for i in range(int(len(sorted_pc1_del_index_list))):
                if len(matched_data_pc1[(sorted_pc1_del_index_list[i] - i*3) : (sorted_pc1_del_index_list[i] - i*3 + 3)]) == 3: # 2024.2.29追加。エラー回避用
