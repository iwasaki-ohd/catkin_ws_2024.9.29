...
# FF_pfの改良
# pcデータが正しくパブリッシュされない問題を修正。

# FF_pf_correct_data_pubの変更ver
# 時間差を一旦無視
# データが２つ揃ったら最新のデータで比較
...
def callback(data, callback_args):
    ...
    rospy.loginfo(f"data_dict['pc1']:{data_dict['pc1']}")
    ...
    #rospy.loginfo(f"最新の要素　data_dict['pc1']:{data_dict['pc1'][timestamp]}")
    
    # pc1とpc2のデータが揃った場合、処理を実行
    if len(data_dict['pc1']) > 0 and len(data_dict['pc2']) > 0:
        data_dict['pc1'] = data_dict['pc1'][timestamp]
        rospy.loginfo(f"---データが２つ揃いました---")
        rospy.loginfo(f"data_dict['pc1']:{data_dict['pc1']}")
        ...
        rospy.loginfo(f"pc1の元データ：{pc_data}")
        ...
        rospy.loginfo(f"pc2の元データ：{pc_data}")
