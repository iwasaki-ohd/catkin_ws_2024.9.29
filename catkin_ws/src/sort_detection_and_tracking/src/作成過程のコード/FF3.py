            ...
            for i in range(0, int(len(matched_data_pc1)/3)):# 検出した人数分ループ
                for j in range(0, int(len(matched_data_pc2)/3)):# 検出した人数分ループ
                    if matched_data_pc1[i * 3] == matched_data_pc2[j * 3]:
                        pc1_double_ID_index.append(i*3)
                        pc2_double_ID_index.append(j*3)
                        rospy.loginfo(f"同一IDを発見。{matched_data_pc1[i * 3]}, {matched_data_pc2[j * 3]}")

            for i in range(0, int(len(pc1_double_ID_index))):
                rospy.loginfo(f"pc1 マッチングリストから削除：{matched_data_pc1[pc1_double_ID_index[i - i*3]]}")
                del matched_data_pc1[pc1_double_ID_index[i - i*3] : pc1_double_ID_index[i - i*3] + 3]
            for j in range(0, int(len(pc2_double_ID_index))):
                rospy.loginfo(f"pc2 マッチングリストから削除：{matched_data_pc2[pc2_double_ID_index[j - j*3]]}")
                del matched_data_pc2[pc2_double_ID_index[j - j*3] : pc2_double_ID_index[j - j*3] + 3]
