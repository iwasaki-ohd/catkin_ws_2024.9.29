...
# データが２つ揃ったら統合作業開始
# データが片側からしか受信できないときは、そのままパブリッシュ(横流し)
...
def callback(data, callback_args):
    ...
    str_pc_data = ",".join([str(_) for _ in parsed_data[1]])
    rospy.loginfo(f"チェック：{str_pc_data}")
    #rospy.loginfo(f"チェック：{str(parsed_data[1])}")
