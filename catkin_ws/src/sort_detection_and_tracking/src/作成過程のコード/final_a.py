        ...
        if matched_data_pc1 is not None and matched_data_pc2 is not None:
            for i in range(0, int(len(matched_data_pc1)/3)):# 検出した人数分ループ
                for j in range(0, int(len(matched_data_pc2)/3)):# 検出した人数分ループ
                    if distance(matched_data_pc1[i*3 + 1 : i*3 +2+1], matched_data_pc2[j*3 + 1 : j*3 +2+1]) <= 0.3:
                        rospy.loginfo("Matching IDs: %d and %d", matched_data_pc1[i*3], matched_data_pc2[j*3])
                        rospy.loginfo(f"距離差：{format(distance(matched_data_pc1[i*3 + 1 : i*3 +2+1], matched_data_pc2[j*3 + 1 : j*3 +2+1]), '.3f')}[m]")
                        matched_data_pc1[i*3] += 100
                        matched_data_pc2[j*3] = matched_data_pc1[i*3]
                        rospy.loginfo("変更後の IDs: %d and %d", matched_data_pc1[i*3], matched_data_pc2[j*3])
                        rospy.loginfo(f"-----------------------------------------------------------")
                        judge = False
        if judge:
