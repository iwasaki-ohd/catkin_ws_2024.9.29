...
class PoseVisualizer:
    ...
    def pose_callback(self, data):
        self.frame_count += 1  # フレーム数を更新
        if time.time() - self.start_time >= 1:  # 1秒経過したら
            self.fps = self.frame_count / (time.time() - self.start_time)  # FPS を計算
            self.frame_count = 0  # フレーム数をリセット
            self.start_time = time.time()  # 開始時間を更新
                ...
                if (char_1 == "rightShoulder" and char_2 == "leftShoulder") or (char_1 == "leftShoulder" and char_2 == "rightShoulder"):
                    if num_1 == num_2:
                        ...
                        if check_id not in check_list: # 重複回避用条件文
                            ...
                            break
        cv2.putText(self.image, f"FPS: {self.fps:.2f}", (10, self.image.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)  # FPS を表示
