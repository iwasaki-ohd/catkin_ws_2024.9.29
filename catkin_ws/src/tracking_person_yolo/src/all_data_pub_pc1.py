...
def bounding_boxes_callback(data):
    global depth_image, bounding_boxes_list, multiple_data_pub, world_point_pub, all_data_pub

    timestamp = rospy.get_time()
    current_time = datetime.fromtimestamp(timestamp)
    only_time = current_time.time()
    """
    # timeの型変更
    ...
    rospy.loginfo("時刻：{}".format(only_time))
...
def main():
    global multiple_data_pub, world_point_pub, all_data_pub
    ...
    # パブリッシャの作成（Unity送信用)
    all_data_pub = rospy.Publisher("all_data_pc1", String, queue_size=10)
    ...
    rospy.Subscriber(
        "/darknet_ros/bounding_boxes", BoundingBoxes, bounding_boxes_callback
    )  # 検出結果を取得
